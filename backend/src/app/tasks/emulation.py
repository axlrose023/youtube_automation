...
async def emulation_task(
    ...
    try:
        ...
        if post_run.get("status") == "stopped" or stop_requested:
            await _finalize_stopped(
                session_id,
                session_store,
                result,
                current_mode,
            ...
            await _finalize_completed(
                ...
                current_mode, current_fatigue, current_personality, completed_at,
            )
            live_payload = await session_store.get(session_id) or {}
            await _persist_safely(
        ...
        await session_store.update(
            session_id, status="failed", stop_requested=False, finished_at=time.time(), error=str(exc),
        )
        live_payload = await session_store.get(session_id) or {}
        await _persist_safely(
