...
class VideoWatcher:
    ...
    def _completed_watch(
        *,
        ...
        if not mark_completed or target_seconds <= 0:
            return False
        tolerance = 1.5 if target_seconds <= 15 else 4.0
        threshold = max(target_seconds - tolerance, target_seconds * 0.92)
        return watched_seconds >= threshold

    async def _delay_interruptible(self, seconds: float) -> float:
    ...
    async def _maybe_continue_current_video(self, profile: WatchProfile) -> bool:
        ...
        if watched_seconds < 1.0:
            self._state.clear_current_watch()
            self._state.on_video_page = True
            return True
        completed = self._completed_watch(
    ...
    async def _execute_watch_focused(self) -> None:
        ...
        total_watched = first_eval_watched + watched_main
        if total_watched < 1.0:
            self._state.clear_current_watch()
            self._state.on_video_page = True
    ...
    async def _watch_for(self, seconds: float, *, mode_a: bool, source_action: str) -> float:
        ...
        while elapsed < seconds:
            if self._state.stop_requested:
                logger.info(
                    "Session %s: stop requested during %s watch loop",
                    self._state.session_id,
                    source_action,
                )
                break
            ...
            if self._state.stop_requested:
                ...
                break
            if comment_depth > 0:
                comment_dwell_seconds += actual_chunk
