...
def build_bootstrap_payload(live_payload: dict) -> dict[str, object]:
    ...
    watched_ads = sanitize_watched_ads(live_payload.get("watched_ads") or [])
    return {
...
def sanitize_watched_ads(watched_ads: list[dict]) -> list[dict[str, object]]:
        ...
        sanitized.append(
            {
                ...
                if key not in {"capture", "capture_id"}
            },
        )
    return sanitized
