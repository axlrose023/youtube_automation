...
from app.services.emulation.session.bootstrap import sanitize_watched_ads

from ..models import EmulationSessionHistory, PostProcessingStatus, SessionStatus
...
def build_status_response(
    ...
    if status == SessionStatus.RUNNING and bool(data.get("stop_requested")):
    ...
    videos_watched, watched_videos_count = derive_watched_video_counters(
        watched_videos,
        fallback_completed=int(data.get("videos_watched", 0) or 0),
        fallback_total=int(data.get("watched_videos_count", 0) or 0),
    )
    ...
    return EmulationSessionStatus(
        ...
        requested_topics=data.get("topics", []),
...
def should_stream_continue(status: EmulationSessionStatus) -> bool:
    if status.status in {SessionStatus.QUEUED, SessionStatus.RUNNING, SessionStatus.STOPPING}:
        return True
    return status.post_processing_status in {
...
async def stream_status_events(
    *,
    initial_status: EmulationSessionStatus,
    get_status: Callable[[], Awaitable[EmulationSessionStatus]],
    ...
    while True:
        ...
        yield f"event: status\ndata: {current_status.model_dump_json()}\n\n"

        if not should_stream_continue(current_status):
            break
...
def build_resume_seed_from_live_payload(data: dict[str, object]) -> ResumeSeed:
    ...
    videos_watched, watched_videos_count = derive_watched_video_counters(
        watched_videos,
        fallback_completed=int(data.get("videos_watched") or 0),
        fallback_total=int(data.get("watched_videos_count") or 0),
    )
    return {
...
def build_resume_seed_from_history(
    ...
    videos_watched, watched_videos_count = derive_watched_video_counters(
        watched_videos,
        fallback_completed=int(history.videos_watched or 0),
        fallback_total=normalized_videos_count(history),
    )
