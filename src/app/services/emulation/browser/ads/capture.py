...
class AdCreativeCapture:
    ...
    async def _start_video_recording(self, page: Page, capture_id: str) -> bool:
        ...
        retryable_statuses = {"no_video", "no_tracks", "not_ready"}
        ...
        while True:
            try:
                payload = await page.evaluate(
                    """async ({ captureId, storeKey, warmupMs }) => {
                        ...
                        const preReadyState = Number(video.readyState || 0);
                        const preCurrentTime = Number(video.currentTime || 0);
                        if (preReadyState < 2 && preCurrentTime <= 0) {
                            return {
                                status: "not_ready",
                                readyState: preReadyState,
                                paused: !!video.paused,
            ...
            if status in {"recording", "already_started"}:
                logger.info(
                    "Ad recorder %s started (%s, mime=%s, readyState=%s, paused=%s, currentTime=%.3f)",
                    capture_id,
                    status,
