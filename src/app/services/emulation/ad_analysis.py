    ...
    async def _analyze_one(
        ...
        try:
            ...
            if size_mb > _MAX_VIDEO_SIZE_MB:
                logger.warning(
                    "Session %s: capture %s video too large (%.1fMB), skipping",
            ...
            result, data = _parse_result(raw)
            summary = json.dumps(data, ensure_ascii=False)

            if result == "relevant":
                await self._uow.ad_captures.update_analysis(
                    capture.id, AnalysisStatus.COMPLETED, summary,
                )
                logger.info(
                    "Session %s: capture %s RELEVANT — %s",
                    session_id, capture.id, data.get("reason", ""),
                )
                return None
            elif result == "not_relevant":
                await self._uow.ad_captures.update_analysis(
                    capture.id, AnalysisStatus.NOT_RELEVANT, summary,
                )
                logger.info(
                ...
                await self._uow.ad_captures.update_analysis(
                    capture.id, AnalysisStatus.SKIPPED, summary,
                )
                logger.warning(
